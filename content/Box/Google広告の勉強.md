---
modified: 2025-03-03T17:49
---

今、受けれるのは9種類ある。
- [x] 検索広告認定資格
- [x] 広告測定認定資格
- [x] ショッピング広告認定資格
- [ ] 動画広告認定資格
- [ ] ディスプレイ広告認定資格





---

### 検索広告
- コンバージョン
	- コンバージョン値とコンバージョン数
- CPA
	- 目標コンバージョン単価
- コンバージョン数の最大化
- 目標コンバージョン単価
- 価値に基づく入札戦略
	- この価値はユーザーの価値で、ユーザーがどのくらいお金を落としてくれるのかの意味？
- VCM 価値重視測定
- LTV
- OCI
- 価値の取り込み
	- コンバージョンの測定？とほぼ同じ意味？
	- どうやってコンバージョンをGoogleに取り込むのか。
		- なおかつ定量的、数値的にできるか
- クリックの最大化
	- ガチで意味ない。とGoogleが言っている
	- 基本的にはコンバージョンの最大化か目標インプレッションシェアに似ておいたほうが良い。
- カバレッジ
	- 網羅率のこと
- 拡張コンバージョン
- オフラインでのコンバージョン測定
- P-MAX キャンペーン
	- Performance Maxの略らしい
	- ALL in Oneの広告
	- 検索、ディスプレイ、動画、などすべてに入れてくれる。
	- 広告の自動化の最終地点
- 動的検索広告
- パフォーマンスプランナー
- レスポンシブ検索広告
- 表示URL？

---

### 広告測定

- 財務目標
	- 戦略的な目標
		- マーケティング目標
			- 広告キャンペーンの目標
- カスタマージャーニー
	- 認知
		- 製品の存在を知る
		- ブランドに関するストーリーテリング
	- 比較検討
		- ブランドを詳しく知ってもらう
		- ブランドの特徴
	- 販売促進
		- 購入してもらう
		- アクションを促す
	- 見込客の獲得
		- 追加購入、紹介してもらう
		- 特典など

> [!NOTE] カスタマージャーニーを計画する
> - **認知:** あなたの会社をまったく知らない人がいるとしたら、どのように会社のことを説明しますか？ 
> 
> - **比較検討:** あなたの会社のことを聞いたことがあるという程度の人に対して、商品やサービスについてどのように説明しますか？  
> 
> - **販売:** 他社の商品ではなく、あなたの会社の商品やサービスを選択するメリットは何ですか？特に優れている点は何ですか？
> 
> - **見込み顧客の獲得:** ユーザーとの関係を深める、またはユーザーにあなたのビジネスを他の人にすすめてもらうには、どうすればよいですか？

- KPI


アトリビューションモデル
[About attribution models - Google Ads Help](https://support.google.com/google-ads/answer/6259715)

拡張コンバージョン
電話のトラッキングに対応するために、転送サービスみたいなのがあるらしい。

データドリブンアトリビューション
マーケティングミックスモデル
マーケティングミックスモデルは、Google広告の機能ではなく一般的なやつ。
Google広告上で何かができるわけではない。
アトリビューションモデルはGoogle広告上の機能

クロスチャネル/マルチタッチアトリビューション
顧客のライフタイムバリュー
コンバージョンリフト測定
ブランド効果測定ではアンケートで測定する

ABテストとインクリメンタリティテスト

メディアミックスの最適化とチャネルの最適化
メディミックスの最適化はどのメディアにどのくらいの予算を割くかなど。
チャネルの最適化はメディアの中で、クリエイティブや入札戦略などを変更すること


### 測定の習慣を身に着ける

- 優先順位を付ける
	- 測定結果を見ることの優先度を高くする
- ダッシュボードを使って、組織のデータを身近なものにする
	- 特にスマホで見れるようにする。
- すべての会議でデータを提示する
	- データは、あらゆる意思決定において中心的な役割を担う必要があります。
- 質問する
	- 「このデータによると、どう対応すべきだと思う？」という質問を相互に投げかける
	- データを用いて検証可能な質問をすることで分析ができる
- テストする
	- 継続的にテストを実施する
	- 何をもって成功なのかはっきりさせておく
- 学ぶ
	- 失敗から学ぶことを心掛ける
	- 失敗してない状態のほうがやばい。



- 自動入札 
- データドリブン アトリビューション
- 広告のローテーション
- ダッシュボード

ビュースルーコンバージョン

コンバージョンのカウント
Google広告ではコンバージョンのカウントは、クリックが行われた日で、そのあとにコンバージョンが発生した場合でも、クリックが行われた日にコンバージョンが行われたと表記される。


コンバージョンの重複を防ぐためオーダーIDなどのトランザクションIDを付ける。


---
### ショッピング広告


リファレンス
- [Explore the 2024 retail guide - Think with Google](https://www.thinkwithgoogle.com/consumer-insights/consumer-journey/retail-guide-and-insights/)
- [Make your products stand out and maximize your reach across Google’s channels - Google Ads Help](https://support.google.com/google-ads/answer/6167176)
- [About advertising with Comparison Shopping Services - Google Merchant Center Help](https://support.google.com/merchants/answer/12653197?visit_id=638762279750564871-1996152853&rd=1)
- [Local inventory ads and free local listings overview - Google Merchant Center Help](https://support.google.com/merchants/answer/14615117?visit_id=638762279750564871-1996152853&rd=1)
- [What makes up a Shopping ad - Google Ads Help](https://support.google.com/google-ads/answer/6275294)
- 



- 小売向けP-Maxキャンペーン
- 通常のショッピングキャンペーン
- どちらのパターンもマーチャントセンターを使う

- 商品ショッピング広告
- ローカル在庫広告
- 無料のリスティング
- P-MAXも通常もCPC（クリック単価）
- EAAとスイスではCSSが必要？

> [!question] Google の通常の検索結果ページにショッピング広告を掲載するためには必ず CSS を使う必要があるのですか？
>はい、ショッピング広告を掲載できるのは CSS だけです。Google ショッピングを使うことも、任意の CSS を選んで使うこともできます。複数の CSS から同時に入札に参加することもできます。 


入札戦略
- コンバージョン値の最大化
- tROAS
- 拡張クリック単価（CPC）
- クリック数の最大化
- 上限クリック単価

ショーケース広告

オムニチャネル

---

### 動画広告

- 動画アクションキャンペーン
- スキップ可能なインストリーム広告
	- 30秒以上視聴した場合のみカウント
	- CPV課金
		- 目標インプレッション単価
		- 目標コンバージョン単価
		- コンバージョン数の最大化
- インフィード広告
	- 検索画面などに出る
	- サムネが表示される
	- CPV課金
- 商品フィードもYouTubeに上げられる
- 複数の広告パターンを試す
- データセグメント（リマーケティング）
	- 自社の顧客データを読み込ませてターゲッティングする
- カスタムオーディエンス
	- Googleの分析したデータを利用してターゲッティングする
- 最適化されたターゲッティング
	- カスタマーマッチ
		- データセグメントのよいやつ
	- 無制限の分類オーディエンス
		- 謎
- YouTubeのチャンネルを視聴したことがある人にアプローチできるらしい
- Insights Finder
	- 特定のユーザーだけ利用可能
- Find My Audience
	- みんな使える
- 入札ソリューション
- 目標インプレッション単価
	- 認知フェーズで使う
- 上限広告視聴単価
- オーディエンスソリューション
	- カスタムオーディエンス
	- 購買移行の強いオーディエンス
	- ライフイベント
	- 無制限の分類オーディエンス
- ブランド効果測定


> [!important] デジタルメディアにおいてはクリエィティブが重要
> クリエイティブの影響度合いはすべてのメディアが49%に対して、
> デジタルは89%と二倍近い
> また、良いクリエイティブは6倍ものROIを記録することもある

動画クリエイティブのABCD
- Attention
	- 臨場感で注意をひく
- Branding
	- ？？？？
- Connection
	- ユーザーの気持ちにフィットさせる
- Direction
	- 視聴者の行動を誘導する


- 比較検討の入札ソリューション
	- 上限CPV
	- セカンドプレイス制なので、これよりかは安くなる。
- オーディエンスソリューション
	- ユーザー属性と詳しいユーザー属性
		- 詳しいユーザー属性は性別や年齢以外の部分
	- アフィニティ カテゴリ
		- 興味や消費行動など
	- カスタムオーディエンス
		- 特定の動画を見た人や、特定の検索をした人
	- 無制限の分類オーディエンス
		- ？？？？？？？？
- ダイナミックラインナップ
	- トピックターゲットと言う名前に変わった？
- 